...
//
// ANSWER: It essentially acts as a private variable like you would see in other languages.  It restricts
// other packages from editing and allows this package to control what gets changed/when/and how.  This controls
// access and makes it simpler to know who can change what variables. 
type ToDo struct {
	toDoMap    DbMap
	dbFileName string
}
...
func (t *ToDo) RestoreDB() error {
	...
	defer dbFile.Close()
	_, err = io.Copy(dbFile, backupFile)
	if err != nil {
		return err
	}
